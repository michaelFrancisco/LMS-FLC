...
CREATE TABLE `manufacturing_order` (
  ...
  `request_production_id` int NOT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_manufacturing_order_request_production1_idx` (`request_production_id`),
  CONSTRAINT `fk_manufacturing_order_request_production1` FOREIGN KEY (`request_production_id`) REFERENCES `request_production` (`id`)
...
INSERT INTO `manufacturing_order` VALUES (40,'finished',72),(41,'finished',76),(42,'finished',78),(43,'finished',82);
/*!40000 ALTER TABLE `manufacturing_order` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `mo_recipe`
--

DROP TABLE IF EXISTS `mo_recipe`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `mo_recipe` (
  `id` int NOT NULL AUTO_INCREMENT,
  `manufacturing_order_id` int NOT NULL,
  `recipe_id` int NOT NULL,
  `quantity` varchar(25) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_mo_recipe_manufacturing_order1_idx` (`manufacturing_order_id`),
  KEY `fk_mo_recipe_recipe1_idx` (`recipe_id`),
  CONSTRAINT `fk_mo_recipe_manufacturing_order1` FOREIGN KEY (`manufacturing_order_id`) REFERENCES `manufacturing_order` (`id`),
  CONSTRAINT `fk_mo_recipe_recipe1` FOREIGN KEY (`recipe_id`) REFERENCES `recipe` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=84 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `mo_recipe`
--

LOCK TABLES `mo_recipe` WRITE;
/*!40000 ALTER TABLE `mo_recipe` DISABLE KEYS */;
INSERT INTO `mo_recipe` VALUES (57,40,76,'1455'),(69,40,76,'1455‬'),(70,40,77,'1940‬'),(71,40,78,'2425‬'),(72,41,79,'1290‬'),(73,42,84,'1000'),(74,42,85,'2000'),(75,42,86,'3000'),(76,42,87,'4000'),(77,42,88,'5000'),(78,42,89,'3000'),(79,43,111,'4000'),(80,43,112,'4500'),(81,43,113,'5000'),(82,43,114,'500'),(83,43,115,'1000');
/*!40000 ALTER TABLE `mo_recipe` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `production` (
  `id` int NOT NULL AUTO_INCREMENT,
  `inventory_Item_ID` int(5) unsigned zerofill NOT NULL,
  `qty` int DEFAULT NULL,
  `unit` varchar(25) CHARACTER SET utf8 COLLATE utf8_bin DEFAULT NULL,
  `size` int DEFAULT NULL,
  `weight` int DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_production_inventory1_idx` (`inventory_Item_ID`),
  CONSTRAINT `fk_production_inventory1` FOREIGN KEY (`inventory_Item_ID`) REFERENCES `inventory` (`Item_ID`)
...
INSERT INTO `production` VALUES (1,00000,60,'g',NULL,100),(2,00000,100,'g',NULL,100),(3,00000,100,'g',NULL,100),(4,00000,144,'g',NULL,100),(5,00000,144,'g',NULL,100),(6,00000,48,'g',NULL,100),(7,00000,100,'g',NULL,100),(8,00000,150,'g',NULL,100),(9,00000,36,'g',NULL,100),(10,00000,36,'g',NULL,100),(11,00000,48,'g',NULL,100),(12,00000,36,'g',NULL,100),(14,00000,48,'g',NULL,100),(16,00000,144,'g',NULL,100),(17,00000,150,'g',NULL,100),(18,00000,36,'g',NULL,100),(19,00000,36,'g',NULL,100),(21,00000,33,'g',NULL,100),(23,00000,50,'g',NULL,100),(24,00000,30,'g',NULL,100);
...
CREATE TABLE `recipe` (
  ...
  `unit` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_recipe_inventory1_idx` (`inventory_Item_ID`),
  CONSTRAINT `fk_recipe_inventory1` FOREIGN KEY (`inventory_Item_ID`) REFERENCES `inventory` (`Item_ID`)
...
INSERT INTO `recipe` VALUES (64,00001,'Ethyl Alcohol FG',5,'Liters'),(65,00001,'IPA',10,'Liters'),(66,00001,'NAOCL',15,'Liters'),(67,00002,'FDC Yellow # 5',20,'Liters'),(68,00002,'Glycerine',25,'Liters'),(69,00002,'Lactic Acid (PURAC)',30,'Liters'),(70,00002,'Little John Texan',35,'Liters'),(71,00002,'Pine Scent HQ',40,'Liters'),(72,00002,'Sodium Lactate (Purasal)',45,'Liters'),(73,00002,'Strong Ammonia water',50,'Liters'),(74,00002,'Tea Tree oil',5,'Liters'),(75,00003,'PEG 40',10,'Liters'),(76,00004,'Bateau TM1887',15,'Liters'),(77,00004,'Grapesunflower',20,'Liters'),(78,00004,'Shower Fresh Scent',25,'Liters'),(79,00005,'Butyl Cellosolve',30,'Liters'),(80,00006,'Chicha Morada',35,'Liters'),(81,00006,'D & C Red # 33',40,'Liters'),(82,00006,'Euniphen',45,'Liters'),(83,00006,'verstatil',50,'Liters'),(84,00007,'D & C Blue #1(1% sol)',5,'Liters'),(85,00007,'Deoplex',10,'Liters'),(86,00007,'Guava Extract',15,'Liters'),(87,00007,'Sodium Chloride',20,'Liters'),(88,00007,'Tongkat ali  extract',25,'Liters'),(89,00007,'Virginity Extract',30,'Liters'),(90,00008,'Baby Fresh Scent',35,'Liters'),(91,00008,'CAPB',40,'Liters'),(92,00008,'Cucumber extract',45,'Liters'),(93,00008,'Guava Extract',50,'Liters'),(94,00008,'MAP',5,'Liters'),(95,00008,'Polysorbate 20',10,'Liters'),(96,00008,'Vitamin E',15,'Liters'),(97,00008,'Witch Hazel extract',20,'Liters'),(98,00009,'Avocado Oil',25,'Liters'),(99,00009,'Bisabolol',30,'Liters'),(100,00009,'GINGER EXTRACT',35,'Liters'),(101,00009,'Mango Extract',40,'Liters'),(102,00009,'Mangosteen Extract',45,'Liters'),(103,00009,'Pentavitin',50,'Liters'),(104,00010,'Camphor',5,'Liters'),(105,00010,'Eucalyptus oil',10,'Liters'),(106,00010,'Menthol',15,'Liters'),(107,00010,'Methyl salicylate',20,'Liters'),(108,00010,'Mineral oil',25,'Liters'),(109,00010,'SLES',30,'Liters'),(110,00010,'Tergitol',35,'Liters'),(111,00012,'Lemon Scent',40,'Liters'),(112,00012,'Oil soluble green',45,'Liters'),(113,00012,'Sampa 17630',50,'Liters'),(114,00012,'Sodium Benzoate',5,'Liters'),(115,00012,'STRAWBERRY SCENT',10,'Liters'),(116,00014,'Blue 606 liquid',15,'Liters'),(117,00016,'Cinchona',20,'Liters'),(118,00016,'Tiki-Tiki',25,'Liters'),(119,00017,'Caramel Color NFH',30,'Liters'),(120,00018,'Citric Acid',35,'Liters'),(121,00018,'Vanilla Flavor',40,'Liters'),(122,00019,'CREAMER',45,'Liters'),(123,00019,'SUGAR',50,'Liters'),(124,00021,'Ferrous Sulphate Hep',5,'Liters'),(125,00023,'Xanthan Gum',10,'Liters'),(126,00024,'Lactic Acid',15,'Liters');
...
CREATE TABLE `request_production` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_request_production_inventory1_idx` (`inventory_Item_ID`),
  CONSTRAINT `fk_request_production_inventory1` FOREIGN KEY (`inventory_Item_ID`) REFERENCES `inventory` (`Item_ID`)
...
INSERT INTO `request_production` VALUES (72,00004,97,'2020-02-08 02:48:44',NULL,NULL,'processing',NULL,NULL),(73,00002,65,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(74,00001,69,'2020-02-08 02:48:44',69,100.00,'moved to inventory',NULL,NULL),(75,00003,80,'2020-02-08 02:48:44',79,98.75,'moved to inventory',NULL,NULL),(76,00005,43,'2020-02-08 02:48:44',NULL,NULL,'processing',NULL,NULL),(77,00006,23,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(78,00007,20,'2020-02-08 02:48:44',NULL,NULL,'processing',NULL,NULL),(79,00008,66,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(80,00009,22,'2020-02-08 02:48:44',NULL,NULL,'moved to inventory',NULL,NULL),(81,00010,50,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(82,00012,100,'2020-02-08 02:48:44',NULL,NULL,'processing',NULL,NULL),(83,00014,53,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(84,00016,67,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(85,00017,44,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(86,00018,50,'2020-02-08 02:48:44',NULL,NULL,'moved to inventory',NULL,NULL),(87,00019,30,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(88,00021,20,'2020-02-08 02:48:44',NULL,NULL,'moved to inventory',NULL,NULL),(89,00023,20,'2020-02-08 02:48:44',NULL,NULL,'moved to inventory',NULL,NULL),(90,00024,2,'2020-02-08 02:48:44',NULL,NULL,'pending',NULL,NULL),(135,00004,10,'2020-02-11 21:12:52',NULL,NULL,'pending','2020-02-12 00:00:00',NULL),(136,00001,10,'2020-02-12 00:40:29',NULL,NULL,'pending','2020-02-12 00:00:00',NULL),(137,00009,10,'2020-02-12 00:43:05',NULL,NULL,'pending','2020-02-12 00:00:00',NULL),(138,00007,10,'2020-02-12 00:47:14',NULL,NULL,'pending','2020-02-12 00:00:00',NULL);
...
CREATE TABLE `system_log` (
  ...
  PRIMARY KEY (`Log_ID`),
  KEY `fk_system_log_users1_idx` (`User_ID`),
  CONSTRAINT `fk_system_log_users1` FOREIGN KEY (`User_ID`) REFERENCES `users` (`User_ID`)
...
INSERT INTO `users` VALUES (00001,'Michael C. Francisco','admin','admin','IT Admin'),(00002,'John','driver','driver','Delivery Agent'),(00007,'Paul','dispensing','dispensing','Dispensing Officer'),(00008,'Ringo','dispatching','dispatching','Dispatching Officer'),(00009,'George','production','production','Production Officer');
...
-- Dump completed on 2020-02-12 11:54:47
